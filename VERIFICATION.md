...
- BRIEFINGの三角形自機を廃止し、本編と同じ13.5px角・45度回転・`#50b9ff`／停止時`#b8fbff`のスクエアへ統一した。
- EVADEはマウス／指の移動へ自機が追従し、敵弾の進行レーンから83px離れて回避する。FREEZEは敵砲口から動く実弾と本体・砲身が入力直後に同時停止し、停止色へ変わる。
- DRAWは押下状態のマウス／指が自機から2体へ進み、同じ進行率でルートとTARGET 1→2を表示する。EXECUTEは自機通過時点で各敵を消し、白シアンの短い命中フラッシュだけを残す。
- PRACTICEはMOVE PAD上の指、実ボタン上のタップ指、Canvas内のマウス／指とゴーストルートを状態別に表示する。実入力開始時または操作完了時に対応案内を消す。
- 説明文とSVG内文字を隠したSTEP 1〜4をWindows Chrome **152.0.7977.82**で画像保存し、入力→移動、停止、経路、通過撃破の順が図形だけで読めることを目視確認した。
- 320×800、360×800、390×844、430×860、844×390、768×800、1280×720でデモ、入力アイコン、ナビゲーションの表示領域を確認。`prefers-reduced-motion`はループを停止し、完了位置・ゴースト線・矢印・命中痕を静止表示する。
- 専用Chromeテストで本編同形状、弾との回避距離、停止色、本体／砲身色一致、停止後の弾位置不変、TARGET順、敵消失タイミング、PRACTICE案内消去、7画面幅、pageerror／console error 0件を確認した。

## 確認の境界

- Android / iOS物理端末は未確認。スマートフォン結果はWindows Chromeのタッチエミュレーションであり、指で隠れる範囲と実機のOSジェスチャーは端末確認が必要。
- 言語非依存の図として制作者目視を行った。完全な初見ユーザーが説明なしで4操作を正答できるかは第三者試遊が必要。

変更: `game.js`, `index.html`, `renderer.js`, `styles.css`, `tests/tutorial-touchpad-browser.cjs`, `README.md`, `VERIFICATION.md`。ファイル削除なし。

---

# 前回の検証記録 — ビジュアルチュートリアル＋仮想タッチパッド
