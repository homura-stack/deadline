...
(async()=>{
 const browser=await chromium.launch({channel:'chrome',headless:true}),report={browser:browser.version(),cases:[],errors:[]},out=path.join(__dirname,'artifacts','task-k');fs.mkdirSync(out,{recursive:true});
 try{
  for(const reduced of [false,true]){
   ...
   await p.evaluate(()=>{const draw=Deadline.Renderer.prototype.draw;Deadline.Renderer.prototype.draw=function(app){window.__kApp=app;draw.call(this,app);if(app.world.failed){
   ...
   await p.locator('#title-start').click();await initialMap(p);await battleReady(p);await p.waitForFunction(()=>!!window.__kApp);
   async function fixture(gauge){await p.evaluate(gauge=>{const a=window.__kApp,w=a.world;w.gauge=gauge;w.phase='normal';w.failed=false;w.life=1;w.player={x:100,y:100};w.bullets=[];w.enemies=[];w.waveGraceRemaining=0;w.safetyRemaining=0;w.passiveRecoveryScale=0;w.events=[];},gauge);}
   ...
   await fixture(99);await p.evaluate(()=>window.__kApp.world.passiveRecoveryScale=1);await p.waitForFunction(()=>Deadline.inspect().readyFx.flashRemaining>0);
   ...
   await fixture(84);await move(p,{x:100,y:100},1);await p.evaluate(()=>window.__kApp.world.bullets=[{id:901,enemyId:0,x:200,y:135,vx:0,vy:0,life:999,grazed:false}]);await move(p,{x:300,y:100});s=await state(p);assert.equal(s.world.gauge,100);assert.equal(s.readyFx.activations,opening+2);assert.equal(s.nearMissFx.gain,16);
   ...
   await p.evaluate(()=>{const w=window.__kApp.world;w.passiveRecoveryScale=1;w.bullets=[{id:902,enemyId:0,x:100,y:100,vx:0,vy:0,life:999,grazed:false}];window.__deathFrames=[];});
   ...
   await fixture(20);await p.evaluate(()=>{const w=window.__kApp.world;w.bullets=[{id:903,enemyId:0,x:100,y:100,vx:0,vy:0,life:999,grazed:false}];window.__deathFrames=[];});await p.waitForFunction(()=>Deadline.inspect().world.failed);
   ...
   await fixture(99);await p.evaluate(()=>window.__kApp.world.passiveRecoveryScale=1);await p.waitForFunction(()=>Deadline.inspect().readyFx.waveRemaining>0);const beforeBlur=(await state(p)).readyFx.activations;
  ...
  assert.deepEqual(report.errors,[]);console.log('PASS TASK K READY transitions, same-step death priority, death timeline, queued retry and reduced motion');
