...
const {detour,approach,planWavePoints}=require('./route-planner.cjs');
...
async function enterGame(page, touchMode = false) {
 ...
 await page.waitForFunction(()=>document.getElementById('title-screen').hidden);await training(page);
 ...
 await page.waitForFunction(()=>document.getElementById('briefing-screen').hidden);await battleReady(page);
}
...
async function charge(page,touchMode=false,cdp=null) {
 if(touchMode) {await page.locator('#restart').tap();await nextArea(page);await padTo(cdp,page,{x:60,y:550});}
 else {await page.locator('#restart').click();await nextArea(page);await move(page,{x:60,y:550});}
 await surviveUntilReady(page);
 const w=(await state(page)).world;assert.equal(w.failed,false);assert.ok(Math.abs(w.gauge-100)<1e-6);assert.ok(w.time>.5);assert.ok(w.bullets.length>=6);
}
...
async function planWave(page){const w=(await state(page)).world,points=planWavePoints(w);
...
async function surviveUntilReady(page,retries=0){
 ...
 const maxIterations=Math.ceil((C.gauge.max-opening.gauge)/C.gauge.recoveryPerSecond*1000/90)+40;
 for(let i=0;i<maxIterations;i++){const w=(await state(page)).world;if(w.failed){assert.ok(retries<6,`Player repeatedly failed while charging Wave ${w.wave}`);await page.keyboard.press(w.timeLimitMultiplier===2?'Digit2':w.timeLimitMultiplier===1.5?'Digit1':'Space');await page.waitForFunction(wave=>Deadline.inspect().world.phase==='normal'&&Deadline.inspect().world.wave===wave,w.wave);return surviveUntilReady(page,retries+1);}if(S.canStop(w))return;
...
if(require.main===module)(async()=>{
 ...
 try{
  ...
  await visitCompleted(page,base+'?debug');await page.waitForLoadState('networkidle');await observe(page);
  assert.equal(await page.title(),'DEAD/LINE — 時間停止ルート');assert.equal(await page.locator('#title-screen').isVisible(),true);assert.equal(await page.locator('.title-heading').getAttribute('aria-label'),'DEAD/LINE');
  assert.ok(await page.locator('#title-art').evaluate(image=>image.complete&&image.naturalWidth===1678&&image.naturalHeight===937));
  ...
  await page.keyboard.down('Space');await page.waitForTimeout(90);assert.equal((await state(page)).titleLeaving,true);assert.equal(await page.locator('#title-screen').isVisible(),true);await page.screenshot({path:path.join(artifacts,'title-start-transition.png')});await page.keyboard.up('Space');await page.waitForFunction(()=>document.getElementById('title-screen').hidden);await training(page);assert.equal((await state(page)).titleActive,false);assert.equal((await state(page)).briefingActive,true);const briefingTime=(await state(page)).world.time;await page.waitForTimeout(180);assert.equal((await state(page)).world.time,briefingTime);assert.equal(await page.locator('.briefing-page').count(),5);await page.locator('#briefing-skip').click();await page.waitForFunction(()=>document.getElementById('briefing-screen').hidden);await battleReady(page);record('Completed START reaches WORLD MAP; the permanent TRAINING route still opens the frozen five-page Pico briefing before Garden Wave 1');
  ...
  await page.screenshot({path:path.join(artifacts,'wave-1-desktop.png'),fullPage:true});record('Wave 1 opens immediately with three visible AIM enemies and a compact 1 / 10 HUD');
   await surviveUntilReady(page);const ready=(await state(page)).world;assert.ok(ready.bullets.length>15);assert.equal(await page.locator('#gauge-value').innerText(),'READY');assert.equal(await page.locator('#status-action').innerText(),'TIME STOP READY');assert.equal(await page.locator('#time-stop-label').innerText(),'TIME STOP');
  ...
  const t=safety.time;await page.waitForTimeout(100);const resumed=(await state(page)).world;assert.ok(resumed.time>t);assert.ok(resumed.gauge>safety.gauge);assert.ok(resumed.safetyRemaining<safety.safetyRemaining);await page.keyboard.press('Space');assert.equal((await state(page)).world.phase,'normal');record('After the run, normal time and recharge resume and a second STOP is gated');
   await charge(page);await page.keyboard.press('Space');const risky=(await state(page)).world;await stroke(page,[risky.player,risky.bullets[0]]);assert.equal((await state(page)).world.failed,false);await page.keyboard.press('Space');await page.waitForFunction(()=>Deadline.inspect().world.failed);const impact=await state(page);assert.ok(impact.deathFx.hitStopRemaining>0);assert.ok(impact.deathFx.flashRemaining>0);assert.ok(impact.damageFx.remaining>0);assert.equal(await page.locator('#retry').isVisible(),false);await page.waitForFunction(()=>Deadline.inspect().deathFx.resultVisible);assert.equal(await page.locator('#retry').isVisible(),true);assert.match(await page.locator('#retry').innerText(),/SPACE\s+RETRY WAVE 1/);assert.match(await page.locator('#result').innerText(),/GAME OVER[\s\S]*WAVE[\s\S]*SCORE[\s\S]*KILLS[\s\S]*TITLE/);assert.ok((await state(page)).audio.plays.damage>=1);await page.keyboard.press('Space');const retriedOpening=(await state(page)).world;assert.equal(retriedOpening.wave,1);assert.equal(retriedOpening.phase,'normal');assert.equal(retriedOpening.gauge,C.waves.retryGaugeInitial);assert.equal(retriedOpening.bullets.length,0);assert.equal(retriedOpening.route,null);assert.ok(retriedOpening.enemies.every(e=>e.alive));assert.equal(await page.locator('#tutorial-prompt').isVisible(),false);record('A risky EXECUTE shows impact before the formal damage result, then retries the same Wave at zero gauge without replaying the tutorial');
  ...
  await charge(page);await page.keyboard.press('Space');const timerRoute=await planSafe(page);await page.evaluate(()=>{window.__done=null;});await page.waitForFunction(()=>window.__done!==null,{},{timeout:6500});assert.ok(S.distance((await page.evaluate(()=>window.__done)).player,timerRoute.at(-1))<1);assert.equal((await state(page)).world.failed,false);record('Nonempty five-second timeout automatically executes the edited route without a timeout failure');
   await page.locator('#restart').click();await nextArea(page);await page.evaluate(()=>{Deadline.config.gauge.recoveryPerSecond=100000;});const waveTypes=[
  ...
  for(let wave=1;wave<=10;wave++){
   if(wave>1){
     if(wave%2===1)await nextArea(page);
   ...
   await page.waitForFunction(()=>Deadline.inspect().world.phase==='wave-clear');if(wave===10)await page.waitForTimeout(180);const cleared=(await state(page)).world;assert.equal(cleared.lastKills,waveTypes[wave-1].length);assert.equal(cleared.bullets.length,0);if(wave%2===0){assert.equal((await state(page)).journey.mode,'restoring');await page.waitForFunction(()=>Deadline.journey.restorationFrame(Deadline.inspect().journey,Deadline.inspect().reducedMotion).complete);assert.match(await page.locator('#area-transition').innerText(),/LIGHT RESTORED/);}else assert.match(await page.locator('#callout').innerText(),wave>=6?/PERFECT EXECUTION/:/ALL CLEAR/);assert.ok(S.distance(cleared.player,allRoute.at(-1))<1);
  }
  await page.waitForFunction(()=>Deadline.inspect().journey.mode==='ending'&&Deadline.inspect().journey.restored===5,{},{timeout:16000});const completed=(await state(page)).world;assert.equal(completed.score,18400);assert.equal(completed.maxChain,10);assert.equal(completed.perfectExecutions,5);assert.equal(completed.hitsTaken,0);assert.equal(completed.totalKills,57);
  assert.equal(await page.locator('#complete').isVisible(),false);assert.equal(await page.locator('#world-map').isVisible(),true);assert.equal(await page.locator('[data-city][data-status="online"]').count(),5);assert.equal(await page.locator('#map-score').innerText(),'18400');await page.screenshot({path:path.join(artifacts,'wave-10-complete-desktop.png'),fullPage:true});assert.equal(await page.locator('#journey-ending').isVisible(),true);await page.locator('#ending-title-button').click();assert.equal((await state(page)).titleActive,true);record('Ten live Waves retain ONE STOP and Wave 7 retry, restore five areas, synchronize the map and reach the ending with a working TITLE command');
   const context=await browser.newContext({viewport:{width:390,height:844},deviceScaleFactor:2,isMobile:true,hasTouch:true});const phone=await context.newPage();hook(phone);await visitCompleted(phone,base+'?debug');await phone.waitForLoadState('networkidle');await observe(phone);await phone.waitForTimeout(700);await phone.screenshot({path:path.join(artifacts,'title-mobile-390x844.png')});await enterGame(phone,true);const cdp=await context.newCDPSession(phone);
  ...
  await charge(phone,true,cdp);await phone.locator('#time-stop').tap();const mobileRoute=await planSafe(phone,true,cdp);await phone.screenshot({path:path.join(artifacts,'gauge-route-mobile.png'),fullPage:true});await phone.locator('#time-stop').tap();await phone.waitForFunction(()=>window.__done!==null);assert.ok(S.distance((await phone.evaluate(()=>window.__done)).player,mobileRoute.at(-1))<1);assert.equal((await state(phone)).world.failed,false);assert.equal((await state(phone)).touchPad.active,false);assert.equal((await state(phone)).touchDraw.cursor,null);assert.equal(await phone.evaluate(()=>scrollY),0);record('Touch executes a safe barrage route to its endpoint with no unintended scrolling');
   ...
   await charge(phone,true,cdp);await phone.locator('#time-stop').tap();const touchRisk=(await state(phone)).world;await padTo(cdp,phone,touchRisk.bullets[0]);await phone.locator('#time-stop').tap();await phone.waitForFunction(()=>Deadline.inspect().world.failed);assert.equal(await phone.locator('#retry').isVisible(),false);await phone.waitForFunction(()=>Deadline.inspect().deathFx.resultVisible);assert.match(await phone.locator('#retry').innerText(),/RETRY WAVE 1/);await phone.locator('#retry').tap();const touchRetry=(await state(phone)).world;assert.equal(touchRetry.wave,1);assert.equal(touchRetry.phase,'normal');assert.equal(touchRetry.gauge,C.waves.retryGaugeInitial);assert.equal(touchRetry.bullets.length,0);record('Touch impact precedes the result, then retry restores the failed Wave at zero gauge with no stale route state');
  ...
  const hudWide=await browser.newPage({viewport:{width:1280,height:720}});hook(hudWide);await visitCompleted(hudWide,base+'?debug');await hudWide.waitForLoadState('networkidle');await enterGame(hudWide);assert.equal(await hudWide.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);assert.equal(await hudWide.locator('.controls').evaluate(element=>element.getBoundingClientRect().bottom<=innerHeight),true);await hudWide.screenshot({path:path.join(artifacts,'hud-normal-1280x720.png'),fullPage:true});await hudWide.close();record('The 1280 x 720 game HUD and one-row command bar remain inside the viewport without horizontal overflow');
  const titleResponsive=await browser.newPage({viewport:{width:1280,height:720}});hook(titleResponsive);for(const viewport of [{width:1280,height:720,name:'title-desktop-16x9.png'},{width:844,height:390,name:'title-mobile-landscape.png'},{width:320,height:800,name:'title-mobile-320.png'},{width:360,height:800,name:'title-mobile-360.png'},{width:768,height:800,name:'title-tablet-768.png'}]){await titleResponsive.setViewportSize(viewport);await visitCompleted(titleResponsive,base);await titleResponsive.waitForLoadState('networkidle');await titleResponsive.waitForTimeout(700);await assertTitleComposition(titleResponsive,viewport);assert.equal(await titleResponsive.evaluate(()=>document.documentElement.scrollWidth<=innerWidth&&document.documentElement.scrollHeight<=innerHeight),true);await titleResponsive.screenshot({path:path.join(artifacts,viewport.name)});}await titleResponsive.close();record('The complete Pico title image and baked logo clear the menu at 16:9, landscape, 320, 360 and 768px');
  const reducedTitle=await browser.newPage({viewport:{width:390,height:844},reducedMotion:'reduce'});hook(reducedTitle);await visitCompleted(reducedTitle,base+'?debug');await reducedTitle.waitForLoadState('networkidle');assert.equal(await reducedTitle.locator('.title-heading').evaluate(element=>getComputedStyle(element).animationName),'none');await reducedTitle.keyboard.press('Space');await reducedTitle.waitForFunction(()=>document.getElementById('title-screen').hidden);await training(reducedTitle);assert.equal((await state(reducedTitle)).titleActive,false);assert.equal((await state(reducedTitle)).briefingActive,true);await reducedTitle.locator('#briefing-skip').click();await reducedTitle.waitForFunction(()=>document.getElementById('briefing-screen').hidden);await battleReady(reducedTitle);await reducedTitle.close();record('Reduced motion shows the title and static BRIEFING immediately and can begin without transition delay');
  const routeVisual=await browser.newPage({viewport:{width:1280,height:900}});hook(routeVisual);await visitCompleted(routeVisual,base+'?debug');await routeVisual.waitForLoadState('networkidle');await observe(routeVisual);await enterGame(routeVisual);
  await routeVisual.evaluate(()=>{Deadline.config.gauge.initial=100;Deadline.config.waves.definitions[0].timeStopSeconds=8;document.getElementById('restart').click();});await nextArea(routeVisual);await routeVisual.keyboard.press('Space');
  ...
  const visual=await browser.newPage({viewport:{width:1280,height:900}});hook(visual);await visitCompleted(visual,base+'?debug');await visual.waitForLoadState('networkidle');await observe(visual);await enterGame(visual);await charge(visual);
  ...
  const local=await browser.newPage({reducedMotion:'reduce'});hook(local);const requests=[];local.on('request',r=>requests.push(r.url()));await visitCompleted(local,'file://'+path.join(__dirname,'..','index.html').replace(/\\/g,'/'));await local.waitForLoadState('load');assert.equal(await local.locator('#title-screen').isVisible(),true);assert.equal(await local.locator('.title-heading').getAttribute('aria-label'),'DEAD/LINE');assert.ok(await local.locator('#title-art').evaluate(image=>image.complete&&image.naturalWidth===1678&&image.naturalHeight===937));assert.equal(await local.locator('#debug').isVisible(),false);assert.equal(await local.evaluate(()=>typeof Deadline.inspect),'undefined');assert.ok(requests.every(url=>url.startsWith('file:')||url.startsWith('data:')));record('Static-file launch loads the supplied Pico title image without external dependencies and hides debug mode');
...
module.exports={state,move,stroke,planWave,surviveUntilReady};
