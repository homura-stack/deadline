...
if(require.main===module)(async()=>{
 ...
 try{
  ...
  assert.ok(await page.locator('#title-art').evaluate(image=>image.complete&&image.naturalWidth===2752&&image.naturalHeight===1536));
  ...
  await page.keyboard.down('Space');await page.waitForTimeout(90);assert.equal((await state(page)).titleLeaving,true);assert.equal(await page.locator('#title-screen').isVisible(),true);await page.screenshot({path:path.join(artifacts,'title-start-transition.png')});await page.keyboard.up('Space');await page.waitForFunction(()=>document.getElementById('title-screen').hidden);await training(page);assert.equal((await state(page)).titleActive,false);assert.equal((await state(page)).briefingActive,true);const briefingTime=(await state(page)).world.time;await page.waitForTimeout(180);assert.equal((await state(page)).world.time,briefingTime);assert.equal(await page.locator('.briefing-page').count(),4);await page.locator('#briefing-skip').click();await page.waitForFunction(()=>document.getElementById('briefing-screen').hidden);await battleReady(page);record('First START offers TRAINING, four-page Pico briefing stays frozen, skip returns to map, then Garden Wave 1');
  ...
  await page.locator('#restart').click();await nextArea(page);const waveTypes=[
