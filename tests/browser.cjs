...
(async()=>{
 ...
 try{
  ...
  assert.match(await page.locator('#complete').innerText(),/MISSION COMPLETE[\s\S]*CLEAR TIME[\s\S]*KILLS[\s\S]*TITLE/);assert.equal(completed.totalKills,57);await page.locator('#complete').evaluate(element=>Promise.all(element.getAnimations().map(animation=>animation.finished)));await page.screenshot({path:path.join(artifacts,'wave-10-complete-desktop.png'),fullPage:true});await page.locator('#game-clear-title').click();assert.equal(await page.locator('#title-screen').isVisible(),true);assert.equal((await state(page)).titleActive,true);record('Ten live Waves teach four barrages, explain ONE STOP, retry an incomplete Wave 7, climax at the formal MISSION COMPLETE and return to TITLE');
