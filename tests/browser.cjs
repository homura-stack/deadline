...
if(require.main===module)(async()=>{
 ...
 try{
  ...
  assert.ok(await page.locator('#title-art').evaluate(canvas=>canvas.width>0&&canvas.height>0));
  ...
  const titleResponsive=await browser.newPage({viewport:{width:1280,height:720}});hook(titleResponsive);for(const viewport of [{width:1280,height:720,name:'title-desktop-16x9.png'},{width:844,height:390,name:'title-mobile-landscape.png'},{width:320,height:800,name:'title-mobile-320.png'},{width:360,height:800,name:'title-mobile-360.png'},{width:768,height:800,name:'title-tablet-768.png'}]){await titleResponsive.setViewportSize(viewport);await titleResponsive.goto(base);await titleResponsive.waitForLoadState('networkidle');await titleResponsive.waitForTimeout(700);const logoBox=await titleResponsive.locator('.title-heading').boundingBox();assert.ok(logoBox.width<=viewport.width-24&&logoBox.height<=viewport.height*.5);assert.ok(logoBox.y>=0&&logoBox.y+logoBox.height<(await titleResponsive.locator('#title-start').boundingBox()).y);assert.equal(await titleResponsive.evaluate(()=>document.documentElement.scrollWidth<=innerWidth&&document.documentElement.scrollHeight<=innerHeight),true);await titleResponsive.screenshot({path:path.join(artifacts,viewport.name)});}await titleResponsive.close();record('Title typography clears the start button and viewport edges at 16:9, landscape, 320, 360 and 768px');
  ...
  const local=await browser.newPage({reducedMotion:'reduce'});hook(local);const requests=[];local.on('request',r=>requests.push(r.url()));await local.goto('file://'+path.join(__dirname,'..','index.html').replace(/\\/g,'/'));await local.waitForLoadState('load');assert.equal(await local.locator('#title-screen').isVisible(),true);assert.equal(await local.locator('.title-heading').getAttribute('aria-label'),'DEAD/LINE');assert.ok(await local.locator('#title-art').evaluate(canvas=>canvas.width>0&&canvas.height>0));assert.equal(await local.locator('#debug').isVisible(),false);assert.equal(await local.evaluate(()=>typeof Deadline.inspect),'undefined');assert.ok(requests.every(url=>url.startsWith('file:')||url.startsWith('data:')));record('Static-file launch loads the title and procedural illustration without external dependencies and hides debug mode');
