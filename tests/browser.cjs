...
async function enterGame(page, touchMode = false) {
 ...
 if(touchMode)await page.locator('#briefing-begin').tap();else await page.keyboard.press('Space');
...
(async()=>{
 ...
 try{
  ...
  await page.keyboard.down('Space');await page.waitForTimeout(90);assert.equal((await state(page)).titleLeaving,true);assert.equal(await page.locator('#title-screen').isVisible(),true);await page.screenshot({path:path.join(artifacts,'title-start-transition.png')});await page.keyboard.up('Space');await page.waitForFunction(()=>document.getElementById('title-screen').hidden);assert.equal((await state(page)).titleActive,false);assert.equal((await state(page)).briefingActive,true);const briefingTime=(await state(page)).world.time;await page.waitForTimeout(180);assert.equal((await state(page)).world.time,briefingTime);await page.keyboard.press('Space');await page.waitForFunction(()=>document.getElementById('briefing-screen').hidden);assert.match(await page.locator('#tutorial-prompt').innerText(),/FREEZE TIME/);record('Transparent DEAD/LINE artwork transitions through a frozen BRIEFING and begins Wave 1 only after a second SPACE');
  ...
  await page.locator('#time-stop').focus();await page.keyboard.down('Space');await page.keyboard.down('Space');await page.keyboard.down('Space');await page.waitForTimeout(100);await page.keyboard.up('Space');
   await page.waitForFunction(()=>Deadline.inspect().timeFx.blend>.25&&Deadline.inspect().timeFx.enterRemaining>0,{},{timeout:400});assert.match(await page.locator('#tutorial-prompt').innerText(),/DRAW THROUGH ENEMIES/);
  ...
  await phone.mouse.click(hybridBox.x+hybridBox.width/2,hybridBox.y+hybridBox.height/2);assert.equal((await state(phone)).world.phase,'stopped');assert.equal(await phone.locator('#time-stop').isDisabled(),true);assert.match(await phone.locator('#hint').innerText(),/SPACEキーで実行/);
