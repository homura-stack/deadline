...
if(require.main===module)(async()=>{
 ...
 try{
  ...
  for(let wave=1;wave<=10;wave++){
   ...
   await page.waitForFunction(()=>Deadline.inspect().world.phase==='wave-clear');if(wave===10)await page.waitForTimeout(180);const cleared=(await state(page)).world;assert.equal(cleared.lastKills,waveTypes[wave-1].length);assert.equal(cleared.bullets.length,0);if(wave%2===0)assert.match(await page.locator('#area-transition').innerText(),/LIGHT RESTORED/);else assert.match(await page.locator('#callout').innerText(),wave>=6?/PERFECT EXECUTION/:/ALL CLEAR/);assert.ok(S.distance(cleared.player,allRoute.at(-1))<1);
  }
  await page.waitForFunction(()=>Deadline.inspect().journey.mode==='map'&&Deadline.inspect().journey.restored===5,{},{timeout:5500});const completed=(await state(page)).world;assert.equal(completed.score,18400);assert.equal(completed.maxChain,10);assert.equal(completed.perfectExecutions,5);assert.equal(completed.hitsTaken,0);assert.equal(completed.totalKills,57);
  assert.equal(await page.locator('#complete').isVisible(),false);assert.equal(await page.locator('#world-map').isVisible(),true);assert.equal(await page.locator('[data-city][data-status="online"]').count(),5);assert.equal(await page.locator('#map-score').innerText(),'18400');await page.screenshot({path:path.join(artifacts,'wave-10-complete-desktop.png'),fullPage:true});await page.locator('#map-title').click();assert.equal((await state(page)).titleActive,true);record('Ten live Waves retain ONE STOP and Wave 7 retry, restore five areas, and return to the map without a TASK D ending');
