...
if(require.main===module)(async()=>{
 ...
 try{
  ...
  assert.ok(await page.locator('#title-art').evaluate(image=>image.complete&&image.naturalWidth===1024&&image.naturalHeight===572));
  ...
  const titleResponsive=await browser.newPage({viewport:{width:1280,height:720}});hook(titleResponsive);for(const viewport of [{width:1280,height:720,name:'title-desktop-16x9.png'},{width:844,height:390,name:'title-mobile-landscape.png'},{width:320,height:800,name:'title-mobile-320.png'},{width:360,height:800,name:'title-mobile-360.png'},{width:768,height:800,name:'title-tablet-768.png'}]){await titleResponsive.setViewportSize(viewport);await titleResponsive.goto(base);await titleResponsive.waitForLoadState('networkidle');await titleResponsive.waitForTimeout(700);await assertTitleComposition(titleResponsive,viewport);assert.equal(await titleResponsive.evaluate(()=>document.documentElement.scrollWidth<=innerWidth&&document.documentElement.scrollHeight<=innerHeight),true);await titleResponsive.screenshot({path:path.join(artifacts,viewport.name)});}await titleResponsive.close();record('The complete Pico title image and baked logo clear the menu at 16:9, landscape, 320, 360 and 768px');
