...
(async () => {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      assert.equal(await page.locator('#briefing-screen').isVisible(), true); assert.equal(await page.locator('.briefing-page').count(), 5);
      ...
      await page.mouse.move(canvas.x + offsetX + target.x * scale, canvas.y + offsetY + target.y * scale); await page.waitForFunction(() => Deadline.inspect().world.phase === 'failed');assert.equal(await page.locator('#result').isVisible(),false);await page.waitForFunction(()=>Deadline.inspect().deathFx.resultVisible);
    ...
    const briefingTime = await page.evaluate(() => Deadline.inspect().world.time); await page.waitForTimeout(220); assert.equal(await page.evaluate(() => Deadline.inspect().world.time), briefingTime); for(let i=0;i<5;i++) await page.keyboard.press('Space'); await page.waitForFunction(() => document.getElementById('briefing-screen').hidden);await battleReady(page); assert.equal(await page.evaluate(() => Deadline.inspect().practice.active),true);
