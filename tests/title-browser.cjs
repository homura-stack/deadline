...
(async () => {
  ...
  try {
    ...
    const reduced = await browser.newPage({ reducedMotion: 'reduce' }); await reduced.goto(base + '?debug'); await reduced.waitForLoadState('networkidle'); assert.equal(await reduced.locator('.title-heading').evaluate(element => getComputedStyle(element).animationName), 'none'); await reduced.keyboard.press('Space'); await reduced.waitForFunction(() => document.getElementById('title-screen').hidden); assert.equal(await reduced.locator('#briefing-screen').isVisible(), true); assert.equal(await reduced.locator('.briefing-demo *').first().evaluate(element=>getComputedStyle(element).animationName),'none'); await reduced.locator('#briefing-skip').click(); await reduced.waitForFunction(() => document.getElementById('briefing-screen').hidden); await reduced.close();
