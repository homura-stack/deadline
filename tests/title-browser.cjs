...
(async () => {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      assert.equal(await page.locator('#briefing-screen').isVisible(), true); assert.equal(await page.locator('.briefing-page').count(), 1);
    ...
    const briefingTime = await page.evaluate(() => Deadline.inspect().world.time); await page.waitForTimeout(220); assert.equal(await page.evaluate(() => Deadline.inspect().world.time), briefingTime); await page.keyboard.press('Space'); await page.waitForFunction(() => document.getElementById('briefing-screen').hidden);await battleReady(page); assert.equal(await page.evaluate(() => Deadline.inspect().practice.active),true);
    const reduced = await browser.newPage({ reducedMotion: 'reduce' }); await reduced.goto(base + '?debug'); await reduced.waitForLoadState('networkidle'); assert.equal(await reduced.locator('.title-heading').evaluate(element => getComputedStyle(element).animationName), 'none'); await reduced.keyboard.press('Space'); await reduced.waitForFunction(() => document.getElementById('title-screen').hidden);await training(reduced); assert.equal(await reduced.locator('#briefing-screen').isVisible(), true); assert.equal(await reduced.locator('.pico-welcome-art img').evaluate(element=>getComputedStyle(element).animationName),'none'); await reduced.locator('#briefing-skip').click(); await reduced.waitForFunction(() => document.getElementById('briefing-screen').hidden);await battleReady(reduced); await reduced.close();
