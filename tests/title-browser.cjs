...
(async () => {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      const logo = await page.locator('.title-logo').boundingBox(), start = await page.locator('#title-start').boundingBox();
      assert.ok(logo.x >= 12 && logo.x + logo.width <= viewport.width - 12); assert.ok(start.y + start.height < viewport.height);
      assert.ok(Math.abs(logo.width / logo.height - 3) < 0.02); assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth && document.documentElement.scrollHeight <= innerHeight), true);
      ...
      await page.locator('#title-start').click(); await page.waitForFunction(() => document.getElementById('title-screen').hidden);
      assert.equal(await page.locator('#briefing-screen').isVisible(), true); assert.equal(await page.locator('.briefing-steps li').count(), 3);
      ...
      await page.locator('#briefing-begin').click(); await page.waitForFunction(() => document.getElementById('briefing-screen').hidden); await page.waitForTimeout(850);
    ...
    await page.locator('#title-start').click(); assert.equal(await page.locator('#title-screen').evaluate(element => element.classList.contains('is-leaving')), true); await page.waitForFunction(() => document.getElementById('title-screen').hidden); assert.equal(await page.evaluate(() => Deadline.inspect().titleActive), false); assert.equal(await page.evaluate(() => Deadline.inspect().briefingActive), true);
    const briefingTime = await page.evaluate(() => Deadline.inspect().world.time); await page.waitForTimeout(220); assert.equal(await page.evaluate(() => Deadline.inspect().world.time), briefingTime); await page.keyboard.press('Space'); await page.waitForFunction(() => document.getElementById('briefing-screen').hidden);
    const reduced = await browser.newPage({ reducedMotion: 'reduce' }); await reduced.goto(base + '?debug'); await reduced.waitForLoadState('networkidle'); assert.equal(await reduced.locator('.title-logo').evaluate(element => getComputedStyle(element).animationName), 'none'); await reduced.keyboard.press('Space'); await reduced.waitForFunction(() => document.getElementById('title-screen').hidden); assert.equal(await reduced.locator('#briefing-screen').isVisible(), true); await reduced.keyboard.press('Space'); await reduced.waitForFunction(() => document.getElementById('briefing-screen').hidden); await reduced.close();
