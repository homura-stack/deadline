...
(async () => {
  ...
  try {
    ...
    for (const viewport of viewports) {
      ...
      await page.locator('#briefing-skip').click(); await page.waitForFunction(() => document.getElementById('briefing-screen').hidden); await page.waitForTimeout(850);
    ...
    const briefingTime = await page.evaluate(() => Deadline.inspect().world.time); await page.waitForTimeout(220); assert.equal(await page.evaluate(() => Deadline.inspect().world.time), briefingTime); for(let i=0;i<4;i++) await page.keyboard.press('Space'); await page.waitForFunction(() => document.getElementById('briefing-screen').hidden); assert.equal(await page.evaluate(() => Deadline.inspect().practice.active),true);
    const reduced = await browser.newPage({ reducedMotion: 'reduce' }); await reduced.goto(base + '?debug'); await reduced.waitForLoadState('networkidle'); assert.equal(await reduced.locator('.title-logo').evaluate(element => getComputedStyle(element).animationName), 'none'); await reduced.keyboard.press('Space'); await reduced.waitForFunction(() => document.getElementById('title-screen').hidden); assert.equal(await reduced.locator('#briefing-screen').isVisible(), true); assert.equal(await reduced.locator('.briefing-demo *').first().evaluate(element=>getComputedStyle(element).animationName),'none'); await reduced.locator('#briefing-skip').click(); await reduced.waitForFunction(() => document.getElementById('briefing-screen').hidden); await reduced.close();
