...
(async()=>{
  ...
  try{
    ...
    await page.locator('#title-start').tap();await page.waitForFunction(()=>document.getElementById('title-screen').hidden);await training(page);assert.equal(await page.locator('.briefing-page').count(),4);
    await page.locator('#briefing-skip').tap();await page.waitForFunction(()=>document.getElementById('briefing-screen').hidden);await battleReady(page);assert.equal(await page.locator('#move-pad').isVisible(),true);
