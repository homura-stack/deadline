...
(async()=>{
  ...
  try{
    ...
    await page.locator('#title-start').tap();await page.waitForFunction(()=>document.getElementById('title-screen').hidden);await training(page);assert.equal(await page.locator('.briefing-page').count(),1);
