...
(async () => {
  ...
  try {
    ...
    await desktop.goto(base + '?debug'); await desktop.waitForLoadState('networkidle'); await desktop.keyboard.press('Space'); await desktop.waitForFunction(() => document.getElementById('title-screen').hidden); await desktop.locator('#briefing-skip').click();
    assert.equal(await desktop.locator('#move-pad').isVisible(), false); const before = await desktop.evaluate(() => Deadline.inspect().world.player), arena = await desktop.locator('#arena').boundingBox();
    ...
    console.log(`PASS Chrome ${browser.version()} tutorial, practice, relative touch pad and 7 responsive viewports`);
    console.log(`PASS precision/fast movement, bounds, DRAW separation, pointer capture and ${fps.toFixed(1)}fps rAF sample`);
