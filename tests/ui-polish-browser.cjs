...
try{
 for(const size of [{width:1920,height:1080},{width:2560,height:1440},{width:1366,height:768}]){
  ...
  await begin(p);await snap(p,`training-${size.width}`);assert.match(await p.locator('#phase-label').innerText(),/TRAINING 1 \/ 4/);
  await plan(p);assert.match(await p.locator('#phase-label').innerText(),/TRAINING 4 \/ 4/);await snap(p,`training-line-${size.width}`);await p.keyboard.press('Space');await map(p,size);
  await p.locator('#map-enter').click();await p.waitForFunction(()=>Deadline.inspect().journey.mode==='battle');await layout(p,size);await snap(p,`battle-${size.width}`);
  ...
  await p.locator('#map-enter').click();await p.waitForFunction(()=>Deadline.inspect().journey.mode==='battle');await surviveUntilReady(p);await p.keyboard.press('Space');const w=(await state(p)).world;await p.waitForFunction(()=>Deadline.inspect().world.stopRemaining<.9);assert.equal(await p.locator('#gauge-value').innerText(),'残りわずか');await snap(p,`time-warning-${size.width}`);await stroke(p,[w.bullets[0]]);await p.keyboard.press('Space');
  await p.waitForFunction(()=>Deadline.inspect().world.failed);await p.waitForTimeout(260);await snap(p,`game-over-${size.width}`);assert.equal(await p.locator('#life').innerText(),'0');assert.equal(await p.locator('#result').isVisible(),true);
  await p.locator('#retry').click();await p.waitForFunction(()=>!Deadline.inspect().world.failed);assert.equal((await state(p)).world.wave,1);assert.equal((await state(p)).world.life,1);
  ...
  report.routes.push({size,flow:'fresh START → 4 pages → 4 practice steps → MAP → Garden W1/W2 → RESTORED → MAP; permanent TRAINING → MAP → Garden → real bullet collision → GAME OVER → RETRY',pico});await p.close();console.log('PASS UI hierarchy, live map pixels, complete original-input routes:',size.width,size.height);
 }
