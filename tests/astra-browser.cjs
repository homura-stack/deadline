...
async function trainingRun(page, index) {
  ...
  const {before,plan,endpoint}=await require('./tutorial-helpers.cjs').plan(page);
  ...
  assert.equal(done.world.totalKills, 3); assert.equal(done.world.score, 950);
  ...
  await page.locator('#training-action').click();
