...
(async()=>{
  ...
  try{
    ...
    await p.reload();await p.waitForLoadState('networkidle');await p.locator('#title-start').click();await map(p);assert.equal((await state(p)).briefingActive,false);
    ...
    await p.close();
    // Leaving an incomplete lesson returns TITLE and remains incomplete after reload.
    const skip=await browser.newPage({viewport:{width:1280,height:720}});await open(skip);await first(skip);
    ...
    const blocked=await browser.newPage({viewport:{width:1280,height:720}});await blocked.addInitScript(()=>Object.defineProperty(window,'localStorage',{get(){throw new DOMException('Blocked','SecurityError');}}));await open(blocked);await first(blocked);await begin(blocked);await plan(blocked);await blocked.keyboard.press('Space');await map(blocked);await blocked.locator('#map-title').click();await blocked.locator('#title-start').click();await map(blocked);assert.equal((await state(blocked)).briefingActive,false);await blocked.close();
